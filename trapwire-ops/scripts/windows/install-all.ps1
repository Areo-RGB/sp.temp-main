...
if (-not $NoBuild) { Invoke-Gradle -AndroidDir $AndroidDir -Args @(':app:assembleDebug') }
$apk = Get-ApkPath $AndroidDir
$devices = Get-AdbDevices
if ($devices.Count -eq 0) { Write-Host "No authorized ADB devices found." -ForegroundColor Yellow; adb devices; exit 1 }
...
Write-Host "Installed on $($devices.Count) device(s)." -ForegroundColor Green
